...
import cv2
import tempfile
from yolo_utils import load_yolo_model, detect_objects

# Constants
MODEL_PATH = "models/seg_model.pt"  # Path to the YOLO model
CLASS_NAMES = ['leftcurve', 'rightcurve', 'uturn']
DISPLAY_WIDTH = 640  # Width of displayed frames


def main():
    
    st.title("YOLO Object Detection - Video seg")
    ...
    if input_type == 'Upload Video':
        ...
        if uploaded_video is not None:
            ...
            while cap.isOpened():
                ...
                objects_frame = detect_objects(model, frame, CLASS_NAMES)

                # Display the objects frame in Streamlit
                stframe.image(objects_frame, channels="RGB")
        ...
        if not cap.isOpened():
            ...
            while cap.isOpened() and not st.session_state.stop_webcam:
                ...
                objects_frame = detect_objects(model, frame, CLASS_NAMES, DISPLAY_WIDTH)
